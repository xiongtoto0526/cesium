 ...
 */
function LZWReader(input, codeSize) {
...
GIFReader.prototype.assert = function (v) {
  if (!v) {
    throw "GIF error - is the data correct?";
  }
};
...
GIFReader.prototype.blockImage = function () {
  ...
  var lct = null;
  if (lctf) {
    lct = this.readColorTable(ctSize);
  }

  var ct = lctf ? lct : this.gct;

  if (this.hasTrans) {
    ct[this.transIdx][3] = 0;
  }
 ...
 * A constuctor func
 *
 * @alias BillboardAnimator
 * @constructor
 */
function BillboardAnimator(options) {
  if (!defined(options)) {
  ...
  this._imageSubRegion = new CallbackProperty(function (time, result) {
    ...
    if (useSceneTime) {
      ...
      secondsElapsed = Math.abs(
        JulianDate.secondsDifference(JulianDate.now(), epoch)
      );
    }
    ...
    result.y =
      (numberOfCols - Math.floor(frame / imagesPerRow) - 1) * frameHeight;
...
Object.defineProperties(BillboardAnimator.prototype, {
  image: {
    get: function () {
      return this._image;
    },
...
/**
 * Creates a image from base image
 * @param {any} options The options param
 * @returns {BillboardAnimator} The back result object.
 */
...
/**
 * Creates a image from gif image
 * @param {any} options The options param
 * @returns {BillboardAnimator} The back result object.
 */
BillboardAnimator.fromGif = function (options) {
  ...
  return when(loadArrayBuffer(url), function (arrayBuffer) {
    ...
    var calculatedFrameRate =
      delay === 0 ? 10 : Math.ceil(1000 / (delay * images.length));
...
export default BillboardAnimator;
